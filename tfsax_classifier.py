...
class TFSAXDictionaryClassifier():
    ...
    def predict(self,X):
        ...
        if self.metric in ['matching','editdistance']:
        ...
        elif self.metric in ['own_dist', 'tlb']:

            dist_mat = self.sax.distance(self.train_words_bps, self.predict_words_bps, int(self.word_length/2), X.shape[1])

            # ed = euclidean_vectorized(scipy.stats.zscore(self.train_data, axis=-1), scipy.stats.zscore(self.train_data, axis=-1))
            ...
            # print((dist_mat/ed).shape)
            # tlb = dist_mat/ed
            # print("TLB: ", np.mean(tlb))
