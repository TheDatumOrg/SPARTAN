...
itrs = ['a4_w2', 'a4_w4', 'a4_w6', 'a4_w8']   # shown on x-axis
ed_method_dir = 'euclidean'
ed_itr = 'euc_kmeans'

# x-tick labels
datasets = [r'$\omega=2$', r'$\omega=4$', r'$\omega=6$', r'$\omega=8$']
methods = list(sym_methods.keys())  # ['SAX','SFA','SPARTAN']

# ---- helpers ----
ACC_CANDIDATES = ('acc', 'accuracy', 'test_acc', 'test_accuracy',  'ri')
...
def _read_csv_avg_acc(dir_path: str):
    ...
    for csv_path in csvs:
        try:
            ...
            continue
        cols = [c for c in df.columns if (c.lower() in ACC_CANDIDATES) or ('ri' in c.lower())]
        if not cols:
            continue
        col = cols[0]
        try:
...
ymin, ymax = 0.66, 0.72  # adjust to your data range
...
save_filename = 'bar_clustering_varying_wordlen_acc_ED_line_lamda0.2.jpg'
