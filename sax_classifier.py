...
from distance_vectorized import hamming_vectorized,symbol_vectorized,sax_mindist,euclidean_vectorized,boss_vectorized,cosine_similarity_vectorized,kl_divergence
...
class SAXDictionaryClassifier():
    ...
    def fit(self,X,y=None):
        ...
        self.sax = SAX(
            ...
            variable_segment=self.variable_segment
        )
    ...
    def predict(self,X):
        ...
        if self.metric in ['editdistance']:
            ...
            n, w = X.shape[-1], self.predict_words_bps.shape[-1]
            sax_param = (self.breakpoints, n, w)
            dist_mat = pairwise_distance(self.predict_words_bps,self.train_words_bps,symmetric=False,metric=self.metric, sax_param=sax_param)
