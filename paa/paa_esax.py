        ...
        for i in range(num_insts):
            ...
            if not self.variable_segment:
                ...
                for j in range(len(segments)):
                    ...
                    min_pt = np.argmin(segments[j])
                    max_pt = np.argmax(segments[j])
                    mid_pt = 0.5*len(segments[j])

                    min_val = segments[j][min_pt]
                    max_val = segments[j][max_pt]
                    mid_val = np.mean(segments[j])
